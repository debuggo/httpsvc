...
BOOL	CIniFile::SectionExist(CString	strSection)
{
	...
	DWORD	dwRetValue;
	dwRetValue = GetPrivateProfileString((LPCTSTR)strSection, NULL, _T(""), chSection, sizeof(chSection)/sizeof(TCHAR), (LPCTSTR)m_strPath);
	return (dwRetValue>0);
}
...
CString		CIniFile::GetKeyValue(CString	strSection, CString		strKey)
{
	...
	dwRetValue = GetPrivateProfileString((LPCTSTR)strSection, (LPCTSTR)strKey,_T(""),chKey,sizeof(chKey)/sizeof(TCHAR),(LPCTSTR)m_strPath);       
	strKeyValue = chKey;
	return	strKeyValue;
}
...
{
	WritePrivateProfileString((LPCTSTR)strSection, (LPCTSTR)strKey, NULL,/*这里写NULL,则删除Key*/(LPCTSTR)m_strPath);
}

void	CIniFile::DeleteSection(CString	strSection)
{
	WritePrivateProfileString((LPCTSTR)strSection, NULL, NULL,/*这里都写NULL,则删除Section*/(LPCTSTR)m_strPath);
}
